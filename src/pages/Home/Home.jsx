          ...
          <div className="d-flex flex-column flex-grow-1 profile-section my-4">
            <div className="d-flex flex-row my-3">
              ...
              <div className="p-name pt-2  px-3 d-flex flex-column">
                <h3 className="name fs-3 font-weight-bold">
                  {myself.data?.first_name} {myself.data?.last_name}
                </h3>
                <h3 className="domain fs-4 fs-md-5 my-2">
                  {myself.data?.domain[0]}
                </h3>
            ...
            <div className="d-flex flex-row my-3 home-profile-bottom">
              ...
              <div className="skill">
                {myself.data?.skills.map((s) => (
                  <div
                    className={`skillset px-2  my-3 skill-${s.level}`}
                    key={s._id}
                  >
                    <p>{s.name}</p>
                  </div>
                ...
                {onbord_project.length !== 0 ? (
                  <>
                    ...
                    {onbord_project.map((p) => (
                      <div
                        className="current-project-title my-2 me-4"
                        key={p._id}
                      >
                        <p className="mb-0 ms-2">
                          <Link to={`/project/show/${p._id}`}>{p.title}</Link>
                        </p>
                        <div className="d-flex justify-content-between">
                          <p className="mb-0 ms-2">
                            {p.createdBy === myself.data?._id
                              ? "Provided "
                              : `End on: ${moment(p.end_date).fromNow()}`}
                          </p>
                          <p className="mb-0 ms-2">
        ...
        <div className="card shadow my-4">
          <div className="card-body">
            <div className="d-flex fs-6 fw-bold">
              <div
                className="mx-4 fs-5 active-toggle pointer"
                id="project"
          ...
          <div className="my-5">
            {users.data !== null &&
              ...
              users.data
                ...
                .slice(0, users.data.length > 10 ? 10 : users.data.length)
                ?.filter((u) => u.admin_verify === true)
                .map((u) => (
                  <ProfileCard
                    user={u}
                    key={u._id}
